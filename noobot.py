    ...
    if keys:
        for key in keys:
            ...
            if table == "device":
                ...
                if control: control_button = types.InlineKeyboardButton(text=text, callback_data=path+"/"+table+"/"+key)
                
                if data[table][key]["is_dimmable"] is True:
                    for to_mtrf_data in ['0', '28', '42', '56', '70', '84', '100', '128']:
                        perip_buttons.append(types.InlineKeyboardButton(text=">>", callback_data=path+"/"+table+"/"+key+":6,1,"+to_mtrf_data))                
                elif data[table][key]["is_dimmable"] is False:
                    perip_buttons.append(types.InlineKeyboardButton(text="Выключить", callback_data=path+"/"+table+"/"+key+":0,0,0"))
                    perip_buttons.append(types.InlineKeyboardButton(text="Включить", callback_data=path+"/"+table+"/"+key+":2,0,0"))
...
def main_keyboard_and_text(message):
    ...
    if message.text == "Дом \U0001F3E0" and (owner or guest):
        ...
        place = types.InlineKeyboardButton(text="Комнаты", callback_data=update_path("place_set"))
        device = types.InlineKeyboardButton(text="Устройства", callback_data=update_path("device_set"))
        preset = types.InlineKeyboardButton(text="Сценарии", callback_data=update_path("preset_set"))
        users = types.InlineKeyboardButton(text="Пользователи", callback_data=update_path("users_set"))
        ...
        elif link == "place_set/correct/place//rename":
            place_id = steps[-2]
            table = steps[-3]
        ...
        elif link == "device_set/add/block":
        ...
        elif link == "device_set/opt/device//rename":
            device_id = steps[-2]
            table = steps[-3]
...
def callback_from_buttons(call):
    ...
    if owner:
        if link == "place_set":
            correct = types.InlineKeyboardButton(text="Редактировать комнату", callback_data=callback[call.from_user.id]+"/correct")
        ...
        elif link == "place_set/correct/place/":
            place_id = steps[3]
        ...
        elif link in ("place_set/correct/place//fill", "place_set/correct/place//fill/device/"):
            place_id = steps[3]
            place_name = data["place"][place_id]["name"]        
            
            if link == "place_set/correct/place//fill": backwards_steps = -1
            elif link == "place_set/correct/place//fill/device/": backwards_steps = -3
            ...
            if keys: 
                if link == "place_set/correct/place//fill":
                    form[call.from_user.id] = list()
                elif link == "place_set/correct/place//fill/device/":
                    device_id = steps[6]
        ...
        elif link == "place_set/correct/place//fill/device//save":
            place_id = steps[3]
            place_name = data["place"][place_id]["name"]        
        ...
        elif link in ("place_set/correct/place//empty", "place_set/correct/place//empty/device/"):
            place_id = steps[3]
            place_name = data["place"][place_id]["name"]
            
            if link == "place_set/correct/place//empty": backwards_steps = -1
            elif link == "place_set/correct/place//empty/device/": backwards_steps = -3
            ...
            if keys:              
                if link == "place_set/correct/place//empty":
                    form[call.from_user.id] = list() 
                elif link == "place_set/correct/place//empty/device/":
                    device_id = steps[6]
                    if device_id in form[call.from_user.id]:
                        form[call.from_user.id].remove(device_id)
                    else:
        ...
        elif link == "place_set/correct/place//empty/device//save":
            place_id = steps[3]
            place_name = data["place"][place_id]["name"]        
            back = types.InlineKeyboardButton(text="Назад \U000021A9", callback_data=backwards_path(callback[call.from_user.id], -4))
            cannel = types.InlineKeyboardButton(text="Отмена \U0000274C", callback_data=callback[call.from_user.id]+"/cannel")
        ...
        elif link == "place_set/correct/place//del":
            place_id = steps[3]
            place_name = data["place"][place_id]["name"]
        ...
        elif link == "device_set/opt/device/":
            device_id = steps[3]
            device_name = data["device"][device_id]["name"] 
        ...
        elif link in ("device_set/opt/device//type", "device_set/opt/device//type/dimer", "device_set/opt/device//type/switch"):
            device_id = steps[3]
            
            if link == "device_set/opt/device//type": backwards_steps = -1
            elif link in ("device_set/opt/device//type/dimer", "device_set/opt/device//type/switch"): backwards_steps = -2
            ...
            if data["device"].get(device_id):
                ...
                if link == "device_set/opt/device//type":
                ...
                elif link in ("device_set/opt/device//type/dimer", "device_set/opt/device//type/switch"):
        ...
        elif link in ("device_set/opt/device//type/dimer/save", "device_set/opt/device//type/switch/save"):
            device_id = steps[3]
            back = types.InlineKeyboardButton(text="Назад \U000021A9", callback_data=backwards_path(callback[call.from_user.id], -3))
            cannel = types.InlineKeyboardButton(text="Отмена \U0000274C", callback_data=callback[call.from_user.id]+"/cannel")            
            keyboard = show_keyboard(service_buttons=[back, cannel])
        ...
        elif link == "device_set/opt/device//del":
            device_id = steps[3]
            device_name = data["device"][device_id]["name"]  
        ...
        elif link == "device_set/opt/device//del/unbind":
            device_id = steps[3]
            CH = data["device"][device_id]["CH"]
            mtrf.tx_command(channel=CH, cmd=9, fmt=0, dat=0)
            accept = types.InlineKeyboardButton(text="Далее \U000027A1", callback_data=callback[call.from_user.id]+"/accept")
            repeat = types.InlineKeyboardButton(text="Повторить отвязку \U0001F504", callback_data=callback[call.from_user.id])
            keyboard = show_keyboard(service_buttons=[accept, repeat]) 
        ...
        elif link == "device_set/opt/device//del/unbind/accept":
            device_id = steps[3]
            device_name = data["device"][device_id]["name"]  
        ...
        elif link == "device_set/add/block/bind":
        ...
        elif link == "device_set/add/block/bind/accept":
        ...
        elif link == "preset_set":
            correct = types.InlineKeyboardButton(text="Редактировать сценарий", callback_data=callback[call.from_user.id]+"/correct")
        ...
        elif link == "preset_set/add/device/":
            device_id = steps[3]
            ...
            if keys:
                ...
                if form[call.from_user.id]["devices"]: buttons = [accept, back, cannel]            
                else: buttons = [back, cannel]            
        ...
        elif link == "preset_set/add/device//accept":
            preset_name = form[call.from_user.id]["preset"]["name"]
            save = types.InlineKeyboardButton(text="Сохранить \U0001F4BE", callback_data=callback[call.from_user.id]+"/save")
            back = types.InlineKeyboardButton(text="Назад \U000021A9", callback_data=backwards_path(callback[call.from_user.id], -1))
            cannel = types.InlineKeyboardButton(text="Отмена \U0000274C", callback_data=callback[call.from_user.id]+"/cannel")            
        ...
        elif link in  ("preset_set/add/device//accept/device/", "preset_set/add/device//accept/device/:,,"):
            device_id = steps[6].split(":")[0]
            back = types.InlineKeyboardButton(text="Назад \U000021A9", callback_data=backwards_path(callback[call.from_user.id], -2))
            cannel = types.InlineKeyboardButton(text="Отмена \U0000274C", callback_data=callback[call.from_user.id]+"/cannel")            
            keys = device_id
            
            if keys and link == "preset_set/add/device//accept/device/:,,":
                dev = steps[6].split(":")
        ...
        elif link == "preset_set/add/device//accept/save":
            preset_name = form[call.from_user.id]["preset"]["name"]
            free_ID = database.free_ID(data, "preset")
            back = types.InlineKeyboardButton(text="Назад \U000021A9", callback_data=backwards_path(callback[call.from_user.id], -5))
            cannel = types.InlineKeyboardButton(text="Отмена \U0000274C", callback_data=callback[call.from_user.id]+"/cannel")
            ...
            owners = types.InlineKeyboardButton(text="Хозяева", callback_data=callback[call.from_user.id]+"/owners")
            guests = types.InlineKeyboardButton(text="Гости", callback_data=callback[call.from_user.id]+"/guests")
            passers = types.InlineKeyboardButton(text="Подписчики", callback_data=callback[call.from_user.id]+"/passers")
        ...
        elif link in ("users_set/owners", "users_set/guests", "users_set/passers"):
            back = types.InlineKeyboardButton(text="Назад \U000021A9", callback_data=backwards_path(callback[call.from_user.id], -1))
            cannel = types.InlineKeyboardButton(text="Отмена \U0000274C", callback_data=callback[call.from_user.id]+"/cannel")           
            
            if link == "users_set/owners":
        ...
        elif link in ("users_set/owners/owner/", "users_set/guests/guest/", "users_set/passers/passer/"):
            table = steps[2]
            user_id = steps[3]
            back = types.InlineKeyboardButton(text="Назад \U000021A9", callback_data=backwards_path(callback[call.from_user.id], -1))
            cannel = types.InlineKeyboardButton(text="Отмена \U0000274C", callback_data=callback[call.from_user.id]+"/cannel")            
            ...
            if data[table].get(user_id):
                ...
                set_owner = types.InlineKeyboardButton(text="Сделать хозяином", callback_data=callback[call.from_user.id]+"/set_owner")
                set_guest = types.InlineKeyboardButton(text="Сделать гостем", callback_data=callback[call.from_user.id]+"/set_guest")
        ...
        elif link in ("users_set/owners/owner//set_guest", "users_set/owners/owner//del",
                      "users_set/guests/guest//set_owner", "users_set/guests/guest//del",
                      "users_set/passers/passer//set_owner", "users_set/passers/passer//set_guest", "users_set/passers/passer//del"):
    ...
    if owner or guest: 
        if link in ("view/place/", "view/place//use/device/", "view/place//use/device/:,,"):
            place_id = steps[2]
            ...
            if data["place"].get(place_id):
                ...
                if keys:
                    ...
                    elif link in ("view/place//use/device/", "view/place//use/device/:,,"):
